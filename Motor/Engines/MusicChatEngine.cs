...
public static class MusicChatEngine
    {
        ...
        private static void StartWindowsMediaScraper()
        {
            ...
            try
            {
                string script = @"Add-Type -AssemblyName System.Runtime.WindowsRuntime -ErrorAction SilentlyContinue; [Console]::OutputEncoding = [System.Text.Encoding]::UTF8; [Windows.Media.Control.GlobalSystemMediaTransportControlsSessionManager, Windows.Media, ContentType = WindowsRuntime] | Out-Null; $req = [Windows.Media.Control.GlobalSystemMediaTransportControlsSessionManager]::RequestAsync(); while($req.Status -eq 0) { Start-Sleep -Milliseconds 10 }; $m = $req.GetResults(); while($true) { try { $s = $null; $sessions = $m.GetSessions(); if ($sessions -and $sessions.Count -gt 0) { for ($k=0; $k -lt $sessions.Count; $k++) { $sess = $sessions[$k]; $p = $sess.GetPlaybackInfo(); if ($p -and $p.PlaybackStatus -eq 4) { $s = $sess; break; } } } if (-not $s) { $s = $m.GetCurrentSession(); } if ($s) { $t = $s.GetTimelineProperties(); $p = $s.GetPlaybackInfo(); $propsReq = $s.TryGetMediaPropertiesAsync(); $waitCount = 0; while($propsReq.Status -eq 0 -and $waitCount -lt 50) { Start-Sleep -Milliseconds 10; $waitCount++; }; if ($propsReq.Status -eq 1) { $i = $propsReq.GetResults(); $art = $i.Artist; $tit = $i.Title; $name = ""Chilling""; if (-not [string]::IsNullOrWhiteSpace($tit)) { if ([string]::IsNullOrWhiteSpace($art)) { $name = $tit; } else { $name = ""$art - $tit""; } } $name = $name -replace ""[\r\n\|]"", """"; $pos = 0; $end = 0; if ($t) { $end = $t.EndTime.TotalSeconds; if ($p -and $p.PlaybackStatus -eq 4) { $diff = ([DateTimeOffset]::Now - $t.LastUpdatedTime).TotalSeconds; $pos = $t.Position.TotalSeconds + $diff; } else { $pos = $t.Position.TotalSeconds; } } $pos = [math]::Round($pos); $end = [math]::Round($end); if ($end -lt $pos) { $end = $pos; } [Console]::WriteLine(""$name|$pos|$end""); } else { [Console]::WriteLine(""Chilling|0|0""); } } else { [Console]::WriteLine(""Chilling|0|0""); } } catch { [Console]::WriteLine(""Chilling|0|0""); } [Console]::Out.Flush(); Start-Sleep -Seconds 1; }";
                string b64 = Convert.ToBase64String(Encoding.Unicode.GetBytes(script));
                var psi = new ProcessStartInfo("powershell", $"-NoProfile -ExecutionPolicy Bypass -EncodedCommand {b64}") { RedirectStandardOutput = true, StandardOutputEncoding = Encoding.UTF8, UseShellExecute = false, CreateNoWindow = true };
        ...
        private static (string Title, double Position, double Length) FetchMusicData()
        {
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows)) {
                ...
                try
                {
                    var psi = new ProcessStartInfo("playerctl", "metadata --format \"{{artist}} - {{title}}\u001f{{position}}\u001f{{mpris:length}}\"") { RedirectStandardOutput = true, UseShellExecute = false, CreateNoWindow = true };
                    using var p = Process.Start(psi);
                    string raw = p?.StandardOutput.ReadToEnd().Trim() ?? "";
                    var parts = raw.Split('\u001f');
                    string r = parts.Length > 0 ? parts[0].Trim() : "";
                    if (!string.IsNullOrEmpty(r) && r != " - ")
                    {
                        double pos = 0, len = 0;
                        if (parts.Length > 1 && long.TryParse(parts[1].Trim(), out long posUs)) pos = posUs / 1_000_000.0;
                        if (parts.Length > 2 && long.TryParse(parts[2].Trim(), out long lenUs)) len = lenUs / 1_000_000.0;
                        return (r, pos, len);
                    }
                }
                catch { }
